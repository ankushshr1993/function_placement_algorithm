...
INVOKER_NODE_NAME=`echo $(kubectl get node --selector='!node-role.kubernetes.io/master' --output=jsonpath={.items..metadata.name})`
kubectl label node $INVOKER_NODE_NAME openwhisk-role=invoker
wsk property set --apihost 34.159.176.225:31001
